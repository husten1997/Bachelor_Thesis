#WIP---------------------------------------------------------------------------------------------------------------------------------------------
AAPL$NivClean_Ind <- index(AAPL$Ratios.PB$NivCleanded[!is.na(AAPL$Ratios.PB$NivCleanded)])

PB_cor <- data.frame(matrix(NA, nrow = 730, ncol = 2))
colnames(PB_cor) <- c("AIC", "BIC")

data.xts <- xts(AAPL$Ratios.PB$NivCleanded[AAPL$NivClean_Ind], order.by = AAPL$NivClean_Ind)
...
data.xts$PB <- lag(AAPL$Ratios.PB$NivCleanded[AAPL$NivClean_Ind], 65)
...
model <- lm(data.xts$P ~ data.xts$PB + lag(data.xts$PB, 1))
summary(model)
AIC(model)
BIC(model)
...
plot(plot.data, type = c("l"), col = c("black", "orange"))
